namespace AMANetworkAnalyzer.Converters;
...
/// <summary>Maps Severity → SolidColorBrush for severity filter badges in drill-down.</summary>
public sealed class SeverityToBadgeBrushConverter : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture) =>
        value is Severity s ? s switch
        {
            Severity.Pass => new SolidColorBrush(Color.FromRgb(0x4C, 0xAF, 0x50)),
            Severity.Info => new SolidColorBrush(Color.FromRgb(0x21, 0x96, 0xF3)),
            Severity.Warning => new SolidColorBrush(Color.FromRgb(0xFF, 0x98, 0x00)),
            Severity.Error => new SolidColorBrush(Color.FromRgb(0xF4, 0x43, 0x36)),
            _ => new SolidColorBrush(Colors.Gray)
        } : new SolidColorBrush(Colors.Gray);
